...
export const UserRoleCgAssetStoreForm: React.FC<UserRoleCgAssetStoreFormProps> = ({
  ...
  const onSubmit = async (data: UserRoleCgAssetStoreFormValues) => {
    try {
      ...
      router.refresh();
      router.push(`/user_role_c_g_asset_stores`);
    ...
    <>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 w-full">
          ...
          <Button disabled={loading} className="ml-auto mr-2" variant="outline" type="button" onClick={() => router.push(`/user_role_c_g_asset_stores`)}>
            キャンセル
          </Button>
