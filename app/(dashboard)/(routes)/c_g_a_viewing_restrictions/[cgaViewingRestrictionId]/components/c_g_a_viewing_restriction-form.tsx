...
export const CGAViewingRestrictionForm: React.FC<CGAViewingRestrictionFormProps> = ({
  ...
  const onSubmit = async (data: CGAViewingRestrictionFormValues) => {
    try {
      ...
      router.refresh();
      router.push(`/c_g_a_viewing_restrictions`);
  ...
  const onDelete = async () => {
    try {
      ...
      toast.error('この閲覧制限を使用中のCGアセット情報が存在します。');
    } finally {
      setLoading(false);
      setOpen(false);
    }
    ...
    <>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 w-full">
          ...
          <Button disabled={loading} className="ml-auto mr-2" variant="outline" type="button" onClick={() => router.push(`/c_g_a_viewing_restrictions`)}>
            キャンセル
          </Button>
