  ...
  const onSubmit = async (data: CGAssetFormValues) => {
    try {
      ...
      if (initialData) {
        ...
        router.refresh();
        router.push(`/c_g_assets/${params.cgAssetId}`);
    ...
    <>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 w-full">
          ...
          <Button disabled={loading} className="ml-auto mr-2" variant="outline" type="button" onClick={() => router.push(`/c_g_assets${(params.cgAssetId !== "new" ? "/" + params.cgAssetId : "")}`)}>
            キャンセル
          </Button>
