...
import {
  CreateApplyGlacierMutation,
  CreateApplyGlacierDocument,
  ApplyDownload,
  CgAsset,
  User,
...
import CGAssetApplyDownloadGlacierFormInput from "./apply-download-glacier-form-input"
import CGAssetApplyDownloadGlacierFormConfirm from "./apply-download-glacier-form-confirm"
...
interface CGAssetApplyDownloadGlacierFormProps {
...
export const CGAssetApplyDownloadGlacierForm: React.FC<CGAssetApplyDownloadGlacierFormProps> = ({
  initialData,
  cgAsset,
  manageUsers,
  setDialogOpen,
  ...
  const onSubmit = async (data: ApplyDownloadFormValues) => {
    try {
      ...
      const ret: FetchResult<CreateApplyGlacierMutation>
        = await apolloClient
          .mutate({
            mutation: CreateApplyGlacierDocument,
            variables: {
              input: {
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <Heading title="データ復元申請" description={`アセットID: ${cgAsset?.asset_id} のデータ復元を申請する`} />
      </div>
      {/* <Separator /> */}
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 w-full">
          {pageNumber === 0 && <CGAssetApplyDownloadGlacierFormInput
            form={form}
            ...
            onNext={onNext}
          />}
          {pageNumber === 1 && <CGAssetApplyDownloadGlacierFormConfirm
            form={form}
            initialData={initialData}
            cgAsset={cgAsset}
            manageUsers={manageUsers}
...
export default CGAssetApplyDownloadGlacierForm