...
type CGAssetSearchAppProdFormValues = z.infer<typeof formSchema>

interface CGAssetSearchAppProdFormProps {
  initialData: CgAssetSearchAppProd | null;
};

export const CGAssetSearchAppProdForm: React.FC<CGAssetSearchAppProdFormProps> = ({
  initialData
}) => {
  ...
  const title = initialData ? 'アセット検索項目: 制作ソフトウェア 編集' : 'アセット検索項目: 制作ソフトウェア 新規追加';
  const description = initialData ? '指定制作ソフトウェアの編集' : '新規制作ソフトウェアの追加';
  const toastMessage = initialData ? '制作ソフトウェアが更新されました。' : '制作ソフトウェアが新規追加されました。';
  ...
  const form = useForm<CGAssetSearchAppProdFormValues>({
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 w-full">
          <div className="md:grid md:grid-cols-3 gap-8">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>制作ソフトウェア表記</FormLabel>
                  <FormControl>
                    <Input disabled={loading} placeholder="制作ソフトウェア 表記" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
          ...
          <Button disabled={loading} className="ml-auto mr-2" variant="outline" type="button"
            onClick={() => router.push(`/admin/c_g_asset_search_app_prods`)}>
            キャンセル
          </Button>
