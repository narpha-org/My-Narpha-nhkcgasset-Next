...
const formSchema = z.object({
  checkAction: z.literal(true, {
    errorMap: () => ({ message: "Boxデータ送付の確認が必要です" }),
  }),
  comment: z
    .string({ required_error: '必須入力', invalid_type_error: '入力値に誤りがります' })
    .max(1000, {
      message: "Boxデータ送付告知コメント は最大 1000 文字以内でご入力ください。",
    }),
});
...
export const CGAssetApplyDownloadBoxDeliverForm: React.FC<CGAssetApplyDownloadBoxDeliverFormProps> = ({
  ...
  const onSubmit = async (data: CGAssetFormValues) => {
    try {
      ...
      toast.success('Boxデータ送付を告知しました。');
    } catch (error: any) {
      toast.error('Boxデータ送付告知に失敗しました。');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <Heading title="Boxデータ送付告知" description={`アセットID: ${initialData?.asset_id} のBoxデータの送付を告知する`} />
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 w-full">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormMessage />
                  <FormDescription>
                    該当アセットのBoxデータ送付に関する番組責任者や一般ユーザへのコメントを入力
                  </FormDescription>
                </FormItem>
              )}
            />
          ...
          <Button disabled={loading} className="ml-auto" type="submit">
            {initialData ? <Save className="mr-2 h-4 w-4" /> : <Plus className="mr-2 h-4 w-4" />} Boxデータ送付を告知する
          </Button>
        </form>
      </Form>
    </>
